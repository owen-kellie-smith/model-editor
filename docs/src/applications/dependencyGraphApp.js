...
function populateVariableDropdown() {
  ...
  const currentSelection = ui.dependencyGraphVariable.value;
  
  // Clear existing options except the first one
  ui.dependencyGraphVariable.innerHTML = '<option value="">Select a variable...</option>';
  ...
  if (ui.dependencyGraphSortAlphabetically.checked) {
    variables.sort();
  }
  ...
  for (const varName of variables) {
    ...
    ui.dependencyGraphVariable.appendChild(option);
  }
  ...
  if (currentSelection && variables.includes(currentSelection)) {
    ui.dependencyGraphVariable.value = currentSelection;
  }
  
  // Enable the controls
  ui.dependencyGraphVariable.disabled = false;
  ui.dependencyGraphDepth.disabled = false;
}
...
function populateDepthDropdown() {
  ui.dependencyGraphDepth.innerHTML = '';
  for (let i = 1; i <= 20; i++) {
    const option = document.createElement('option');
    option.value = i;
    option.textContent = i;
    ui.dependencyGraphDepth.appendChild(option);
  }
  // Set default to 2
  ui.dependencyGraphDepth.value = '2';
}
...
function applyFitToScreen() {
  const fitToScreen = ui.dependencyGraphFitToScreen.checked;
  if (fitToScreen) {
    ui.dependencyGraphSvg.classList.add('fit-to-screen');
  } else {
    ui.dependencyGraphSvg.classList.remove('fit-to-screen');
  }
}
...
function showDownloadLinks() {
  ui.downloadDependencySvg.style.display = 'inline-block';
  ui.downloadDependencyPng.style.display = 'inline-block';
}
...
function hideDownloadLinks() {
  ui.downloadDependencySvg.style.display = 'none';
  ui.downloadDependencyPng.style.display = 'none';
}
...
function downloadSvg(event) {
  ...
  const svgElement = ui.dependencyGraphSvg.querySelector('svg');
  if (!svgElement) {
    return;
  }
  ...
  link.href = url;
  const filename = sanitizeFilename(ui.dependencyGraphVariable.value);
  link.download = `dependency-graph-${filename}.svg`;
...
function downloadPng(event) {
  ...
  img.onload = function() {
    ...
    canvas.toBlob(function(pngBlob) {
      ...
      link.href = pngUrl;
      const filename = sanitizeFilename(ui.dependencyGraphVariable.value);
      link.download = `dependency-graph-${filename}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
...
async function generateGraph() {
  const modelEnv = getModelEnv();
  if (!modelEnv || !modelEnv.features) {
    ui.dependencyGraphDot.textContent = 'No model loaded';
    ui.dependencyGraphSvg.innerHTML = '';
    hideDownloadLinks();
    return;
  }
  
  const variable = ui.dependencyGraphVariable.value;
  if (!variable) {
    ui.dependencyGraphDot.textContent = 'Please select a variable';
    ui.dependencyGraphSvg.innerHTML = '';
    hideDownloadLinks();
    return;
  }
  
  const depth = parseInt(ui.dependencyGraphDepth.value, 10);
  ...
  try {
    ...
    ui.dependencyGraphDot.textContent = dotSource;
    
    // Try to render SVG
    try {
      const svg = await renderDotToSvg(dotSource);
      ui.dependencyGraphSvg.innerHTML = svg;
      // Apply fit-to-screen setting after rendering
      applyFitToScreen();
      // Show download links after successful render
      showDownloadLinks();
    } catch (svgError) {
      ui.dependencyGraphSvg.innerHTML = `<p style="color: orange;">SVG rendering not available: ${svgError.message}</p><p>Install viz.js to enable SVG rendering, or copy the DOT source above to your own renderer.</p>`;
      hideDownloadLinks();
    }
  } catch (error) {
    ui.dependencyGraphDot.textContent = `Error: ${error.message}`;
    ui.dependencyGraphSvg.innerHTML = '';
    hideDownloadLinks();
  }
}
...
/**
 * Wire up the dependency graph UI handlers
 */
export function wireDependencyGraphHandlers() {
  // Populate depth dropdown once on load
  populateDepthDropdown();
  
  // Wire change events to automatically generate graph
  ui.dependencyGraphVariable.addEventListener('change', () => {
    if (ui.dependencyGraphVariable.value) {
      generateGraph();
    }
  });
  
  ui.dependencyGraphDepth.addEventListener('change', () => {
    if (ui.dependencyGraphVariable.value) {
      generateGraph();
    }
  });
  ...
  ui.dependencyGraphFitToScreen.addEventListener('change', applyFitToScreen);
  
  // Add event listener for sort alphabetically checkbox
  ui.dependencyGraphSortAlphabetically.addEventListener('change', () => {
    populateVariableDropdown();
  });
  
  // Add event listeners for download links
  ui.downloadDependencySvg.addEventListener('click', downloadSvg);
  ui.downloadDependencyPng.addEventListener('click', downloadPng);
  ...
  window.addEventListener('modelLoaded', () => {
    populateVariableDropdown();
    ui.dependencyGraphDot.textContent = '';
    ui.dependencyGraphSvg.innerHTML = '';
    hideDownloadLinks();
  });
}
