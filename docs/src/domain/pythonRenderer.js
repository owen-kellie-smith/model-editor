...
export function renderModelAsPython(modelObj, features) {
  ...
  lines.push("    ap.add_argument('--traj-point', action='store_true', help='Draw only the moving point (not the whole tail).')");
  lines.push("    ap.add_argument('--traj-head-color', type=str, default='red', help='Default head color for trajectories.')");
  ...
  lines.push("    # ---- Trajectory GIF (optional) ----");
  lines.push("    if bool(getattr(args, 'plot', False)):");
  lines.push("        traj_spec = str(getattr(args, 'traj', '')).strip()");
  lines.push("        if not traj_spec:");
  ...
  lines.push("                return out");
  lines.push("");
  lines.push("            head_map = _parse_color_map(str(getattr(args, 'traj_head_colors', '')))");
  lines.push("            tail_map = _parse_color_map(str(getattr(args, 'traj_tail_colors', '')))");
  lines.push("");
  ...
  lines.push("                return out");
  lines.push("");
  lines.push("            trajs = _parse_trajs(traj_spec)");
  ...
  lines.push("                dd = dd[[tcol] + parts].dropna().sort_values(tcol)");
