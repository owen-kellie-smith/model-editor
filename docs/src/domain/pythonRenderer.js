...
lines.push("    ap.add_argument('--gif', type=str, default=default_gif, help='GIF output path (default: <model_id>.gif)')");
  ...
  lines.push("    ap.add_argument('--traj-t', type=str, default=TEMPORAL_ID, help='Temporal column to animate over for --traj (defaults to temporal index).')");
  ...
  lines.push("            )" );
  // ---- Trajectory GIF block (generated Python) ----
  lines.push("    if bool(getattr(args, 'plot', False)):" );
  lines.push("        traj = str(getattr(args, 'traj', '')).strip()" );
  lines.push("        if traj:" );
  lines.push("            try:" );
  ...
  lines.push("" );
  lines.push("            df = pd.read_csv(str(args.csv))" );
  lines.push("            cols = [c.strip() for c in traj.split(',') if c.strip()]" );
  lines.push("            if len(cols) != 3:" );
  lines.push("                raise SystemExit('--traj expects exactly 3 columns: x,y,z')" );
  lines.push("            x3, y3, z3 = cols" );
  ...
  lines.push("            out_gif = str(getattr(args, 'gif', 'model_out.gif'))" );
  lines.push("            fps = int(getattr(args, 'fps', 24))" );
  lines.push("            dpi = int(getattr(args, 'dpi', 120))" );
  lines.push("" );
  lines.push("            for c in [tcol, x3, y3, z3]:" );
  lines.push("                if c not in df.columns:" );
  lines.push("                    raise SystemExit(f'Column {c} not found. Available columns: {list(df.columns)}')" );
  lines.push("" );
  lines.push("            dtraj = df[[tcol, x3, y3, z3]].dropna().sort_values(tcol)" );
  lines.push("            # If multiple rows per t due to other indices, take the first per t." );
  lines.push("            # For a specific slice, pass --index ... to filter during computation." );
  lines.push("            dtraj = dtraj.groupby(tcol, as_index=False).first()" );
  lines.push("            if dtraj.empty:" );
  lines.push("                raise SystemExit('No trajectory points found after filtering.')" );
  lines.push("" );
  lines.push("            fig = plt.figure()" );
  lines.push("            ax3d = fig.add_subplot(111, projection='3d')" );
  lines.push("" );
  lines.push("            frame_idx = list(range(0, len(dtraj), step))" );
  lines.push("            if frame_idx[-1] != len(dtraj) - 1:" );
  lines.push("                frame_idx.append(len(dtraj) - 1)" );
  lines.push("" );
  lines.push("            x_min, x_max = float(dtraj[x3].min()), float(dtraj[x3].max())" );
  lines.push("            y_min, y_max = float(dtraj[y3].min()), float(dtraj[y3].max())" );
  lines.push("            z_min, z_max = float(dtraj[z3].min()), float(dtraj[z3].max())" );
  lines.push("" );
  lines.push("            head_color = str(getattr(args, 'traj_head_color', 'red'))" );
  lines.push("            tail_color = str(getattr(args, 'traj_tail_color', 'blue'))" );
  lines.push("" );
  lines.push("            def draw(i):" );
  lines.push("                ax3d.clear()" );
  lines.push("                j = frame_idx[i]" );
  lines.push("" );
  lines.push("                if point_only:" );
  lines.push("                    seg = dtraj.iloc[[j]]" );
  lines.push("                else:" );
  lines.push("                    seg = dtraj.iloc[: j + 1]" );
  lines.push("" );
  lines.push("                # Tail (path so far)" );
  lines.push("                if not point_only:" );
  lines.push("                    ax3d.plot(seg[x3].values, seg[y3].values, seg[z3].values, color=tail_color, linewidth=2)" );
  lines.push("" );
  lines.push("                # Head (current point)" );
  lines.push("                ax3d.scatter([dtraj.iloc[j][x3]], [dtraj.iloc[j][y3]], [dtraj.iloc[j][z3]], color=head_color, s=60)" );
  lines.push("" );
  lines.push("                ax3d.set_xlabel(x3)" );
  lines.push("                ax3d.set_ylabel(y3)" );
  lines.push("                ax3d.set_zlabel(z3)" );
  ...
  lines.push("        else:" );
  lines.push("            if bool(getattr(args, 'plot', False)):" );
  lines.push("                print('Plot requested but --traj not provided; skipping plot.')" );
