...
| R7 | Calculate variable precedents from formulae | `model.test.js::when model contains precedents` | Precedents (variables that each variable depends on) are extracted from formulae, excluding language functions
| R8 | Calculate variable dependents from formulae | `model.test.js::when model contains dependents` | Dependents (inverse of precedents) are calculated from formulae, showing which variables depend on each variable |
