...
3. Load or paste a `model.xml` (e.g. from the [examples folder](docs/examples))
4. Inspect Sample Evaluation which is a rendering of the model with sample inputs made up by the editor.
5. Inspect the graph of variable dependencies.
6. Edit variables (individually or in the main Model textarea) and see how your edits change the Sample Evaluation etc.
7. Export as spreadsheet (which adds sample inputs that you can change in your spreadsheet editor).
8. Export as Python. [Run the Python script](docs/examples/python) possibly with [actual inputs to replace the sample inputs](docs/examples/annuity-model/demo).
...
```bash
npm install
node ./src/cli/index.js validate docs/examples/restaurant-model/model.xml
node ./src/cli/index.js graph docs/examples/restaurant-model/model.xml --root MONTHLY_NET_PROFIT --depth 2
node ./src/cli/index.js export-python docs/examples/restaurant-model/model.xml
node ./src/cli/index.js export-spreadsheet docs/examples/restaurant-model/model.xml
```
...
npm install
npm link
model-editor validate docs/examples/restaurant-model/model.xml
model-editor export-python docs/examples/restaurant-model/model.xml
model-editor export-spreadsheet docs/examples/restaurant-model/model.xml
```

#### Getting help

Run any of the following to list all available commands:
...
model-editor commands:
  Usage: model-editor validate <model.xml>
  Usage: model-editor graph <model.xml> --root <VARIABLE[,VARIABLE2,...]> [--depth <n>] [--out <file>]
  Usage: model-editor export-python <model.xml> [--out <file.py>]
  Usage: model-editor export-spreadsheet <model.xml> [--out <file.xlsx>]
  Usage: model-editor functions

Run 'model-editor functions' to list all built-in standard functions.
```

Individual subcommands do not have their own `--help` flag; passing incorrect or missing arguments prints the usage line for that command.
...
```
model-editor export-python <model.xml> [--out <file.py>]
```
...
```
model-editor export-spreadsheet <model.xml> [--out <file.xlsx>]
```
...
Both commands validate the input model before exporting and report a clear error message if the file is missing or invalid.
...
- **Model files** (`model.xml`) - Define variables, their relationships, calculations, and any custom functions

### What files can you write with the model-editor?

You can export:
- **XML files** (`.xml`) - Export model definitions
...
src/                    # All application source code
  core/                 # Core business logic (model, renderers, etc.)
  utils/                # Shared utilities (helpers, logger, formatters, etc.)
  browser/              # Browser-specific app code
    applications/       # Feature modules (model, graph, CRUD)
  cli/                  # CLI entry point
